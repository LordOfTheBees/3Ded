		...
		while (m_window->pollEvent(event)) {
			if (event.type == sf::Event::KeyPressed) {
				...
				case sf::Keyboard::H: {
					...
					std::cout << "<W> (wrong side) - on/off rendering of polygons whose normals are collinear with the direction of the camera (ìrevoluteî polygons)\n";
	...
	while (m_window->isOpen()) {
		while (m_window->pollEvent(event)) {
			...
			if (event.type == sf::Event::KeyPressed) {
				...
				case sf::Keyboard::H: {
					std::cout << "\t=======Helper=======\n";
					std::cout << "\t<N> - create a new polygon. First, you want to answer the query in the console ñ the number of points in the polygon. Then it takes the mouse to select the points and press <Enter>\n";
		...
		while (m_window->pollEvent(event)) {
			...
			if (event.type == sf::Event::KeyPressed) {
				...
				case sf::Keyboard::H: {
					...
					std::cout << "\t\t<C> - change color. You will need to respond to the request in the console. Color in the format RGB (R, G, and B range from 0 ñ 255).\n";
					std::cout << "\t\t====================\n";
					break;
				}
				default:
